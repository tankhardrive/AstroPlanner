...
/// <summary>Az/Alt point on a custom horizon profile. Uses named properties so System.Text.Json serializes correctly.</summary>
...
/// <summary>
/// A custom horizon defined by (Azimuth, Altitude) pairs.
/// Azimuth is 0–360° measured north through east (standard astronomical convention).
/// Altitude is 0–90°.
/// </summary>
public class HorizonProfile
{
    ...
    public double GetAltitudeAt(double azimuth)
    {
        ...
        if (upperIdx == -1)
        {
            // azimuth is after the last point — wrap to first
            var p1 = Points[n - 1];
            var p2 = Points[0];
        ...
        if (upperIdx == 0)
        {
            // azimuth is before the first point — wrap from last
            var p1 = Points[n - 1];
            var p2 = Points[0];
